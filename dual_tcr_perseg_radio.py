...
RADIO-space Scoring Configuration (Recommended)

This version uses RADIO feature space for TCR arbitration.
Performance: 93.42% mIoU on PerSeg benchmark (best)

Key advantages:
- Multi-teacher semantic supervision (CLIP, DINOv2, SAM)
- Better semantic coverage assessment
- Sharp discrimination for rejecting background distractors
...
def get_arguments():
    parser = argparse.ArgumentParser(description='Dual-TCR with RADIO-space Scoring')
    parser.add_argument('--data', type=str, default='./data', help='Path to PerSeg dataset')
    parser.add_argument('--outdir', type=str, default='dual_tcr_radio_scoring', help='Output directory')
...
def evaluate_mask_consistency_sam(mask, test_feat_sam, target_emb_sam):
    """Score mask in SAM feature space."""
...
def evaluate_mask_consistency_radio(mask, test_feat_radio, target_emb_radio):
    """Score mask in RADIO feature space (semantic) - achieves 93.42% mIoU."""
...
def persam(args, obj_name, images_path, masks_path, output_path):
    ...
    for test_idx in tqdm(range(len(os.listdir(test_images_path)))):
        ...
        # ================= TRONG TAI TOI CAO (TCR) - RADIO Space =================
        # RADIO-space scoring achieves 93.42% mIoU (better than SAM-space 92.43%)
        score_base = evaluate_mask_consistency_radio(final_mask_base, test_feat_radio, target_emb_radio)
        score_radio = evaluate_mask_consistency_radio(final_mask_rad, test_feat_radio, target_emb_radio)
